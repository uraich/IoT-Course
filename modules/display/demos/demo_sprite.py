...
class BouncingSprite(object):
    ...
    def draw(self):
        ...
        if prev_x > x:
            # Left
            self.display.draw_filledRectangle(x + w, prev_y, x_speed, h, 0)
        elif prev_x < x:
            # Right
            self.display.draw_filledRectangle(x - x_speed, prev_y, x_speed, h, 0)
        if prev_y > y:
            # Upward
            self.display.draw_filledRectangle(prev_x, y + h, w, y_speed, 0)
        elif prev_y < y:
            # Downward
            self.display.draw_filledRectangle(prev_x, y - y_speed, w, y_speed, 0)
