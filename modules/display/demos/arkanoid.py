...
import fonts.NeatoReduced5x7 as NeatoReduced5x7
...
class Ball(object):
    ...
    def clear(self):
        """Clear ball."""
        self.display.draw_filledRectangle(self.x, self.y, self.width, self.height, 0)

    def clear_previous(self):
        """Clear prevous ball position."""
        self.display.draw_filledRectangle(self.prev_x, self.prev_y,
...
class Paddle(object):
    ...
    def h_position(self, x):
        ...
        if(x != self.x):  # Check if paddle moved
            ...
            if x > prev_x:
                self.display.draw_filledRectangle(prev_x, self.y,
                                        x - prev_x, self.height, 0)
            else:
                self.display.draw_filledRectangle(x + self.width, self.y,
                                        (prev_x + self.width) -
                                        (x + self.width),
                                        self.height, 0)
        else:
...
class Powerup(object):
    ...
    def clear_previous(self):
        ...
        self.display.draw_filledRectangle(self.x, self.prev_y,
                                self.width, self.height, 0)
...
class Score(object):
    ...
    def reset(self):
        """Reset score."""
        self.value = 0
        self.display.draw_filledRectangle(self.x, self.y,
