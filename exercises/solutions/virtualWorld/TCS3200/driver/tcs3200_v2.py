...
class TCS3200(object):
    ...
    def __init__(self, OUT=19, S2=5, S3=18, S0=None, S1=None, LED=None,OE=None):
        ...
        self._meas_time = 1000 # measure for 1000 ms
        self._cycle = 0
    ...
    @property
    def meas_duration(self):
        return self._meas_time

    @meas_duration.setter
    def meas_duration(self,duration):
        if duration < 100 or duration > 5000:
            print("Measurement duration should be in the range 100ms .. 5000ms")
            return
        self._meas_time = duration
    ...
    def meas(self):
        ...
        self._tim.init(period=self._meas_time, mode=Timer.ONE_SHOT, callback=self._stop_meas_handler)
            
    def _stop_meas_handler(self,src):
        if self._debug:
            print("Measurement ended")
            print("Found {:d} cycles on the TCS3200 OUT pin".format(self._cycle))
    ...
    def meas_freqs(self):
        ...
        for i in range(self.CLEAR_COMP+1):
            ...
            self.meas()       # start the measurement
            while self._meas:
                time.sleep_ms(10)
    ...
    def measured_freq(self):
        frequency = 1000 * self._cycle/self._meas_time   # measurement time is given in ms
        return frequency
