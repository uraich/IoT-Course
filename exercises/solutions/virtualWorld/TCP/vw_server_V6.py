...
class vw_server():
    ...
    def createVirtualWorld(self):
        ...
        self.paper=box(length=.1, width=10,height=5, pos=vector(-2,0,0)) #Create the object that will represent your target (which is a colored card for our project)
        ...
        self.distanceLabel = label(pos=vector(0,7,0), text='Target Distance is: ', box=False, height=20)
    ...
    def serve(self) :
        ...
        while True:
            ...
            print("Distance: {:f}, colors: red: 0x{:02x}, green: 0x{:02x}, blue: 0x{:02x}".format(
                distance,
                colors[colorComponent['red']],
                colors[colorComponent['green']],
                colors[colorComponent['blue']]))
            
            # send acknowledge
            self._conn.send("acq\r\n".encode())
            # if the distance is small enough, update the paper colors
            # the colors change with distance to the color sensor
            # We therefore take the color with the highest color intensities seen
            current_color = self.paper.color
            if current_color.x < colors[colorComponent['red']]/255.0 and \
               current_color.y < colors[colorComponent['red']]/255.0 and \
               current_color.z < colors[colorComponent['red']]/255.0 :
                self.paper.color = vector(colors[colorComponent['red']]/255.0,
                                          colors[colorComponent['green']]/255.0,
                                          colors[colorComponent['blue']]/255.0)
            
            # change the position of the virtual paper according to the distance
            if distance > 40:
                self.paper.color=vector(0,0,0)  # set paper color tp black
                distance = 100 # make sure the paper is out of sight
                self.distanceLabel.text="No paper"
            else:
                self.distanceLabel.text="Distance to sensor: " + "{:04.2f}".format(distance) + "cm"
            self.paper.pos=vector(distance/4.0-4.0,0,0)
