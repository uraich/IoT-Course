...
for i in range(256):
    fitted[i] = coeff[0]*math.pow(calib[i],5)+coeff[1]*math.pow(calib[i],4)+coeff[2]*math.pow(calib[i],3)+ coeff[3]*math.pow(calib[i],2) +coeff[4]*calib[i] + coeff[5] + 5
    # print(fitted[i])
