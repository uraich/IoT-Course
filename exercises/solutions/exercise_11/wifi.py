...
def connect(ssid=None,password=None,hostname=None):
    encoding          ='utf-8'
    ssid_1            = "WLAN1xxxxxxxxxx"
    password_1        = "WLAN1pwpwpwpwpwpw"
    ssid_2            = "WLAN22xxxxxxxxxx"
    password_2        = "WLAN2pwpwpwpwpw"
    ...
    if ssid == None:
        for i in range(len(accessPoints)):
            if str(accessPoints[i][0],encoding) == ssid_1:
                print("Connecting to ",ssid_1)
                ssid = ssid_1
                password = password_1
                break
            elif str(accessPoints[i][0],encoding) == ssid_2:
                print("Connecting to ",ssid_2)
                ssid = ssid_2
                password = password_2
                break
